...
- **发现**：展示 GitHub 图库目录，**点卡片可预览图库的图片**（已安装的读本地图库，没装的用目录里的预览图，点图可开原图）；点「安装」下载 Release ZIP、校验 SHA-256 并自动切换，**下载期间卡片上有进度条**（上游给了 `content-length` 显示百分比，没给则显示滚动条纹 + 已下载字节）；已安装的条目不显示按钮（更新在图库页）
