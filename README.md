...
- **发现**：展示 GitHub 图库目录，**点卡片可预览图库的图片**（已安装的读本地图库，没装的用目录里的预览图，点图可开原图）；卡片上还会显示该图库的下载量（服务端代拉 GitHub Release 的 `download_count`，同一图库的多版本相加，缓存 30 分钟；清单热链的包没有 Release 可数就不显示）。点「安装」下载 Release ZIP、校验 SHA-256 并自动切换，**下载期间卡片上有进度条**（上游给了 `content-length` 显示百分比，没给则显示滚动条纹 + 已下载字节）；已安装的条目不显示按钮（更新在图库页）
